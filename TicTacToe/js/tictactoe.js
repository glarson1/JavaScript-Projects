...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if (activePlayer === 'X') {
            ...
            select.style.backgroundImage = 'url("images/up1.jpg")';
            //Active player may only be 'X' or 'O' so, if not 'X' it must be 'O'
        } else {
            //If activePlayer is equal to 'O', the o.png is placed in HTML.
            select.style.backgroundImage = 'url("images/down1.jpg")';
        }
...
function checkWinConditions() {
    ...
    //X 6, 7, 8 condition.
    else if (arrayIncludes('6X', '7X', '8X')) { drawWinLine(50, 508, 100, 508) }
    //X 0, 3, 6 condition.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    function animateLineDrawing() {
        ...
        if (x1 <= x2 && y1 <= y2) {
            ...
            if (y < y2) { y += 10; }
            //This condidion cancels our animation loop
...
function resetGame() {
    ...
    for (let i = 0; i < 9; i++) {
        //This variable gets the html elemet of i.
